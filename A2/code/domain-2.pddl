(define (domain MineBot)
    (:types
        MineBot cell ore hammer rock lift chargestation
    ) 
    ...
    (:action MoveClear
        ...
        :precondition (and (Location ?m ?c1) (Connect ?c1 ?c2)(Clear ?m)(>= (battery ?m) 1)(not(= ?c1 ?c2)))
        :effect (and (Location ?m ?c2) (not(Location ?m ?c1)) (decrease (battery ?m) 1))
    )

    (:action MoveHold
        :parameters (?m - MineBot ?c1 - cell ?c2 - cell)
        :precondition (and (Location ?m ?c1) (Connect ?c1 ?c2)(not(Clear ?m))(>= (battery ?m) 1)(not(= ?c1 ?c2)))
